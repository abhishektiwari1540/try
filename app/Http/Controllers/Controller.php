...
class Controller
{
    ...
    public function setContent($to = null,$name = null,$action = null,$other = null) {
        $settingsEmail          =  Config('Site.from_email');
        $emailActions           =  EmailAction::where('action', '=',$action)->get()->toArray();
        if(count($emailActions) > 0){
            $emailTemplates     =  EmailTemplate::where('action', '=',$action)->select("name", "action","subject","body")->get()->toArray();
            ...
            $messageBody     =  str_replace($constants, $other, $emailTemplates[0]['body']);
            $SendEMail       =  explode(",",$to);
            
            foreach ($SendEMail as $key => $emailSend) {
                if($emailSend != null){
                    $this->sendMail($emailSend,$name,$subject,$messageBody, $settingsEmail);
                }
            }
        }
    }
    ...
    public function userRoleType($user_role_id='')
    {
        $user_type = '';
        if($user_role_id == 1){
